...
Finally, the analyzed datasets are read using `do_harvest_doFuture` in parallel and stored in an `.rds` file suffixed with date and time (to prevent over-writing the previously saved harvest files). Furthermore, using the function `harvest_cleanup` (of @sec-report-clean-harvest), a clean, abridged version of the raw harvest dataframe (only containing the important outcome measures of interest) is made and saved in another `.rds` file (again suffixed with date and time).
...
It should be noted that `plot_caterpillar` requires the complete (i.e., raw) dataset of harvested results.
