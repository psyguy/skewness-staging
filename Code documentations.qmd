...
The harvesting component was directly implemented in the parallel function `do_harvest_doFuture` described in @sec-pipeline-wrapper-functions.
...
The function `fit_extract` gets an `.rds` file and extracts the `unstandardized` and `stdyx.standardized` *M*plus parameter estimates and the additional book-keeping information saved by `do_fit_doFuture` and stores them in a dataframe.
...
The function `do_harvest_doFuture` is a wrapper around `fit_extract` that uses the package `doFuture` as a parallelization backend to `foreach`. It gets a list of `.rds` files (that can be taken from or `fit_refs`, or by using `list.files` function in the directory in which the analysis results are saved) and returns a large dataframe including parameter estimates (and additional information) extracted by `fit_extract`.
...
The following is the code we used to run the whole pipeline, which consists of two chunk: One that creates the book-keeping reference files (using functions described in @sec-bookkeeping), and another that runs the whole simulation study (using functions described in @sec-pipeline-functions).
