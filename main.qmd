...
*M*plus then saves the output files (`.out` and `.gh5`).
After the analysis, `run_MplusAutomation` extracts parameter estimates and returns them in an `R` object, which, with some additional variables, are saved in separate `.rds` files using wrapper functions described in @sec-pipeline-wrapper-functions.
...
#### Analysis outputs file references
